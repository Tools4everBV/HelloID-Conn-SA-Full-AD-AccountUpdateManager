...
'@ 
Invoke-HelloIDGlobalVariable -Name "ADmanagerGroups" -Value $tmpValue -Secret "False" 
$tmpValue = @'
...
Invoke-HelloIDDatasource -DatasourceName "AD-user-generate-table-managers" -DatasourceType "4" -DatasourceInput $tmpInput -DatasourcePsScript -$tmpPsScript -DatasourceModel $tmpModel -returnObject ([Ref]$dataSourceGuid_2) 
...
Invoke-HelloIDDynamicForm -FormName "AD Account - Update manager" -FormSchema $tmpSchema  -returnObject ([Ref]$dynamicFormGuid) 
<# END: Dynamic Form #>
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD Account - Update manager" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-user-secret" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-user-update-manager" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD Account - Update manager' already exists. Nothing to do with the Delegated Form task..." 
}
